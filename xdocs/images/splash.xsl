  ...
  <svg id="body" width="492" height="150" viewBox="0 0 492 150">
    ...
    <defs>
      ...
      <radialGradient id="backgroundGradient" r=".7" cx="0.5">
        ...
        <filter id="dropShadow" primitiveUnits="objectBoundingBox" 
                x="-.2" y="-.2" width="1.4" height="1.4">
    ...
    <g id="content" filter2="url(#dropShadow)">
      ...
      <g id="topText" font-family="'20th Century Font', 'Futura XBlk BT'" 
        font-size="75" text-anchor="middle" 
      ...
      <xsl:if test="$revisionType != 'revisionType'">
        <g id="betaStamp" transform="translate(416, 95) rotate(-30) scale(.7)" filter="url(#dropShadow)">
          <rect x="-90" y="-32" width="180" height="40" stroke="red" stroke-width="5" fill="none" />
          <text font-family="'20th Century Font'" text-anchor="middle" font-size="45" fill="red" ><xsl:value-of select="substring-after($revisionType, 'revisionType')" /><xsl:text> </xsl:text><xsl:value-of select="substring-after($revisionNumber, 'revisionNumber')"/></text>
        </g>
      </xsl:if>
    </g>
