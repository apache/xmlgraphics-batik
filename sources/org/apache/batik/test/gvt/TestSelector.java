...
public class TestSelector implements Selector {
    ...
    /*
     * Checks the event to see if it is a selection gesture and processes it accordingly.
     * @param evt the GraphicsNodeEvent, which may be a "select gesture"
     * Param evt is a GraphicsNodeEvent rather than a GraphicsNodeMouseEvent for
     * future extension, so we can use Shift-arrow, etc.
     */
	...
	if (isSelectContinueGesture(evt)) {
		...
		if (selectionNode != source) { // have been dragged into new node!
