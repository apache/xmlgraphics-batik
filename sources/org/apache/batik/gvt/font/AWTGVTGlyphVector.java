...
public class AWTGVTGlyphVector implements GVTGlyphVector {
    ...
    /**
     * Calculates the logical bounds for each glyph. The logical bounds are what
     * is used for highlighting the glyphs when selected.  
     */
    private void computeGlyphLogicalBounds() {
        ...
        for (int i = 0; i < getNumGlyphs(); i++) {
            ...
            if (glyphVisible[i]) {
                ...
                if (glyphTransform == null) {
                    ...
                    tempLogicalBounds[i] = new Rectangle2D.Double(glyphX, 
								  glyphY,
								  glyphWidth, 
								  glyphHeight);
			...
			maxHeight = glyphHeight;
                    ...
                    // store three corner points so we can determine whether the
                    // glyph is rotated
        ...
        if (fullBounds.getHeight() < maxHeight*1.5) {
			...
			tempLogicalBounds[i].getBounds2D();
    ...
    public int getCharacterCount(int startGlyphIndex, int endGlyphIndex) {
        ...
        int charCount = 0;
        for (int i = ci.getBeginIndex() + startGlyphIndex;
	     i <= ci.getBeginIndex() + endGlyphIndex; 
	     i++) {
